...
class ObjectsAttributesBinarizationTF{
...
public function __construct($typ) {
    ...
    echo Date("H:i:s")." starting ObjectsAttributesBinarizationTF\n<br/>";
    if($typ=="antikvariat"){
   ...
   /* mysql_query("SET character_set_results=cp1250");
    ...
    mysql_query("TRUNCATE objects_binary_attributes_tf");
    ...
    echo Date("H:i:s")." finishing ObjectsAttributesBinarizationTF\n<br/>";    
}
private function CalculateTF_IDF(){
  ...
  /*  $query_all_docs = "select count(distinct `oid`) as `oids` from `objects_binary_attributes_tf` where 1 "; 
    $result = mysql_query($query_all_docs);
    while ($row = mysql_fetch_array($result)) {  
        $all_docs = $row["oids"];
    } 
    */
    $query_doc_lenght = "select `oid`, sum(`value`) as `features` from `objects_binary_attributes_tf` where 1 group by `oid`"; 
    $result = mysql_query($query_doc_lenght);
    while ($row = mysql_fetch_array($result)) {  
        $oid_doc_lenght[$row["oid"]] = $row["features"];
    } 
    
  /*  $query_idf = "select `feature`, sum(`value`) as `values` from `objects_binary_attributes_tf` where 1 group by `feature`"; 
   ...
   // print_r($feature_idf);
    ...
    $query_oid_feature_val = "select *  from `objects_binary_attributes_tf` where 1 "; 
    ...
    foreach ($oid_feature_value as $oid => $features) {
        foreach ($features as $feature => $value) {
          ...
          //  $tf_idf = $tf*$idf;
            $update = "update `objects_binary_attributes_tf` set `value`=$tf where `oid`=$oid and `feature`=\"$feature\" limit 1";
          //  echo $update."\n<br/>";
            mysql_query($update);
        }
    }
...
private function CreateBinnryAttributesAntikvariat($row){
    ...
    $query_insert_features = "INSERT INTO `objects_binary_attributes_tf`(`oid`, `feature`, `value`) VALUES ";
    $first = 1;
    $binTerms = array();
    $used_terms = array();         
